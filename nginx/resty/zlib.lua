...
local function initInflate(stream, windowBits)
    ...
    windowBits = windowBits or (15 + 32) -- +32 sets automatic header detection
...
function _M.inflateGzip(input, output, bufsize, windowBits)
    bufsize = bufsize or DEFAULT_CHUNK
...
function _M.compress(data, bufsize)
    ...
    local function _input()
        ...
        local str = _sub(data, i, j)
        return #str > 0 and str or nil
    end

    local function _output(str)
        _insert(output, str)
    end

    -- compress the data
    local ok, err = _M.deflateGzip(_input, _output, bufsize)
...
function _M.uncompress(data, bufsize)
    ...
    -- uncompress the data
    local ok, err = _M.inflateGzip(_input, _output, bufsize)
    if not ok then return nil, err end
